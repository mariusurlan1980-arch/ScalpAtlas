    ...
    try {
      ...
      if (payload.type === 'RESULT') {
        ...
        let result: AnalysisResult = timeframeRef.current
          ? rawResult
          : { ...rawResult, expiry: null };

        // v0.4.5 — ANTI-FLICKER LIVE.
        // Un BUY/SELL direct trebuie văzut de 3 ori consecutiv înainte de afișare.
        // La 2 secunde/cadru înseamnă aproximativ 6 secunde de consistență.
        if (rawResult.signal === 'BUY' || rawResult.signal === 'SELL') {
          ...
          if (nextCount < 3) {
            result = {
              ...
              bias: dir,
              expiry: null,
              reason: `Confirmare LIVE ${nextCount}/3 pentru ${dir}. Aștept consistența semnalului înainte de confirmare.`,
            };
          } else if (nextCount === 3) {
            // Alertă o singură dată la confirmarea 3/3; nu repetăm la 4/3, 5/3 etc.
        ...
        if (isWait) {
          ...
          const suffix = candidateCount > 0 && candidateCount < 3 ? ` • ${candidateCount}/3` : '';
            ...
            <>
              ...
              <View style={styles.resultCard}>
                ...
                <Text style={styles.status}>
                  Cameră: {cameraReady ? 'PREGĂTITĂ' : 'INIȚIALIZARE'} • Motor: {engineReady ? 'PREGĂTIT' : 'INIȚIALIZARE'} • TF: {detectedTimeframe || 'AUTO'} • Scanare: ~2 sec • Confirmare: 3 cadre • Alarmă: ON
                </Text>
              </View>
...
const styles = StyleSheet.create({
  ...
  livePill: { flexDirection: 'row', alignItems: 'center', gap: 7, borderWidth: 1, borderRadius: 12, paddingHorizontal: 10, paddingVertical: 7 },
