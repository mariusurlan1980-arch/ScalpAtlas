...
export default function LiveAnalysisApp() {
  ...
  const [message, setMessage] = useState('Poziționează camera pe grafic și blochează cadrul.');
  ...
  const startLive = () => {
    ...
    setLive(true);
    setMessage('LIVE pornit. Graficul este urmărit automat.');
    void captureAndAnalyze(false);
  };
  ...
  const stopLive = () => {
    ...
    setLocked(false);
    setAnalysis(null);
    setMessage('LIVE oprit. Repoziționează graficul și blochează cadrul.');
  };
            ...
            <>
              ...
              <View style={styles.actions}>
                <Pressable style={[styles.lockButton, locked && styles.lockButtonActive]} onPress={toggleLock}>
                  <Text style={styles.actionText}>{locked ? 'DEBLOCHEAZĂ' : 'BLOCHEAZĂ GRAFICUL'}</Text>
                </Pressable>

                {!live ? (
                  <Pressable
                    style={[styles.primaryButton, (!locked || !cameraReady || !engineReady) && styles.disabled]}
                    onPress={startLive}
                  >
                ...
                <Pressable
                  style={[styles.scanButton, (!locked || scanning) && styles.disabled]}
                  onPress={() => void captureAndAnalyze(true)}
                >
                  <Text style={styles.actionText}>SCANEAZĂ ACUM</Text>
                </Pressable>
              ...
              <View style={styles.resultCard}>
                ...
                <Text style={styles.status}>
                  Cameră: {cameraReady ? 'PREGĂTITĂ' : 'INIȚIALIZARE'} • Motor: {engineReady ? 'PREGĂTIT' : 'INIȚIALIZARE'} • Scanare automată: ~10 sec
                </Text>
              </View>
