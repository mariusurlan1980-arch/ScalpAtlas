...
import { LANGUAGE_NAMES, SUPPORTED_LANGUAGES, type SupportedLanguage, useAppLocalization } from './localization';
...
const GUIDE_KEY = 'scalpAtlas.live.guide.v0418.count';
...
export default function LiveAnalysisApp() {
  const { t, languageCode, regionCode, systemLanguage, languageMode, setLanguage } = useAppLocalization();
  ...
  const onAnalyzerMessage = (event: AnalyzerMessageEvent) => {
    try {
      ...
      if (payload.type === 'READY') {
        ...
        if (!ok) setMessage(t('atlasIncomplete'));
        return;
      }
      ...
      if (payload.type === 'RESULT') {
        ...
        if (isWait) {
          ...
          setMessage(result.reason ? localizeReason(result.reason) : t('noSignal'));
        }
  ...
  const localizeReason = (reason?: string | null) => {
    ...
    if (!value) return '';
    if (value === 'Imagine insuficient de clară') return t('imageUnclear');
    if (value.startsWith('Graficul ocupă o zonă prea mică')) return t('chartTooSmall');
    if (value === 'Direcția nu este suficient de clară') return t('directionUnclear');
    if (value.startsWith('Prețul este sub rezistență')) return t('belowResistance');
    if (value.startsWith('Prețul este deasupra suportului')) return t('aboveSupport');
    if (value === 'Potrivirea cu atlasul este prea slabă sau semnalele se contrazic') return t('atlasWeak');
    return value;
  };
    ...
    <View key={item.id} style={styles.historyCard}>
      <View style={styles.historyTop}>
        <Text style={[
          ...
          {item.signal === 'WAIT' ? `${t('wait')}${item.bias ? ' ' + item.bias : ''}` : item.signal}
        </Text>
      ...
      <Text style={styles.historySmall}>{t('expiry')}: {item.expiry ? `${item.expiry} min` : '—'}</Text>
      <Text style={styles.historySmall}>{t('direction')}: {item.directionMin && item.directionMax ? `${item.directionMin}–${item.directionMax} min` : '—'}</Text>
      {!!item.reason && <Text style={styles.historySmall}>{localizeReason(item.reason)}</Text>}
        ...
        <>
          <View style={styles.outcomeRow}>
            {(['WIN', 'LOSS', 'PENDING'] as Outcome[]).map((outcome) => (
              ...
              >
                <Text style={styles.outcomeText}>{outcome === 'PENDING' ? t('unevaluated') : outcome}</Text>
              </Pressable>
            ))}
          </View>
          <TextInput
    ...
    <View style={styles.safe}>
      <View style={styles.header}>
        <View>
          ...
          <Text style={styles.subtitle}>LIVE • {SCALP_ATLAS_COUNT} {t('models')} • v0.4.18 • {languageCode.toUpperCase()}{regionCode ? `-${regionCode}` : ''}</Text>
        </View>
        <View style={styles.headerActions}>
          <Pressable style={styles.languageButton} onPress={() => setShowLanguagePicker(true)}>
            <Text style={styles.languageButtonText}>{languageMode === 'auto' ? 'AUTO' : languageCode.toUpperCase()}</Text>
            <Text style={styles.languageButtonSub}>{languageCode.toUpperCase()}{regionCode ? `-${regionCode}` : ''}</Text>
          </Pressable>
        ...
        <ScrollView contentContainerStyle={styles.page}>
          ...
          {!permission ? (
            ...
            <>
              {showGuide && (
                <View style={styles.guideCard}>
                  <View style={styles.guideHeader}>
                    ...
                    <Text style={styles.guideVersion}>v0.4.18</Text>
                  </View>
              ...
              >
                ...
                {confirmationBadgeStyle && (
                  ...
                  >
                    ...
                    >
                      {analysis?.bias ? `${t('confirmation')} ${analysis.bias}` : t('confirmation')}
                    </Text>
                  </View>
                )}
              ...
              <View style={styles.resultCard}>
                ...
                {analysis ? (
                  <>
                    ...
                    <Text style={styles.reason}>{localizeReason(analysis.reason)}</Text>
                  </>
                ) : (
              ...
              <Text style={styles.clientFooter}>SCALP ATLAS • v0.4.18 • {t('disclaimer')}</Text>
            </>
          )}
        </ScrollView>
      )}
        ...
        <ScrollView contentContainerStyle={styles.listPage}>
          <View style={styles.sectionHeader}>
            <View>
              ...
              <Text style={styles.sectionSub}>{t('localRecords', { count: history.length })}</Text>
            </View>
      ...
      {screen === 'STATS' && (
        <ScrollView contentContainerStyle={styles.listPage}>
          ...
          <View style={styles.statsGrid}>
            ...
            <View style={styles.statCard}><Text style={styles.statValue}>{stats.avg}%</Text><Text style={styles.statLabel}>{t('avgScore')}</Text></View>
          ...
          <View style={styles.detailCard}>
            <Text style={styles.detailLabel}>{t('topModel')}</Text>
            <Text style={styles.detailValue}>{stats.topPattern}</Text>
          </View>
          <View style={styles.detailCard}>
            <Text style={styles.detailLabel}>{t('topTimeframe')}</Text>
            <Text style={styles.detailValue}>{stats.topTf}</Text>
          </View>
          <Text style={styles.disclaimer}>{t('statsDisclaimer')}</Text>
        </ScrollView>
      )}
      ...
      >
        <View style={styles.languageOverlay}>
          <View style={styles.languageCard}>
            ...
            <Pressable
              style={[styles.languageOption, languageMode === 'auto' && styles.languageOptionSelected]}
              onPress={() => { void setLanguage('auto'); setShowLanguagePicker(false); }}
            >
              <Text style={styles.languageOptionText}>AUTO • {LANGUAGE_NAMES[systemLanguage]}</Text>
              <Text style={styles.languageOptionCode}>{systemLanguage.toUpperCase()}</Text>
            </Pressable>
            ...
            <ScrollView style={styles.languageList} contentContainerStyle={styles.languageListContent}>
              {SUPPORTED_LANGUAGES.map((code: SupportedLanguage) => (
                <Pressable
                  key={code}
                  style={[styles.languageOption, languageMode === 'manual' && languageCode === code && styles.languageOptionSelected]}
                  onPress={() => { void setLanguage(code); setShowLanguagePicker(false); }}
                >
                  <Text style={styles.languageOptionText}>{LANGUAGE_NAMES[code]}</Text>
...
const styles = StyleSheet.create({
  ...
  languageButton: { minWidth: 54, borderWidth: 1, borderColor: '#33445c', borderRadius: 10, paddingHorizontal: 8, paddingVertical: 5, alignItems: 'center', backgroundColor: '#0b111a' },
  languageButtonText: { color: '#e9f4ff', fontSize: 10, fontWeight: '900', letterSpacing: 0.4 },
  languageButtonSub: { color: '#6f859d', fontSize: 7, fontWeight: '800', marginTop: 1 },
  languageOverlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.72)', alignItems: 'center', justifyContent: 'center', padding: 18 },
  languageCard: { width: '100%', maxWidth: 520, maxHeight: '80%', borderRadius: 16, borderWidth: 1, borderColor: '#26364a', backgroundColor: '#0a1018', padding: 14 },
  languageHeader: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 },
  languageTitle: { color: '#eef6ff', fontSize: 15, fontWeight: '900', letterSpacing: 0.6 },
  languageSub: { color: '#7f93aa', fontSize: 10, marginTop: 2 },
  languageClose: { width: 34, height: 34, borderRadius: 17, alignItems: 'center', justifyContent: 'center', backgroundColor: '#111a25' },
  languageCloseText: { color: '#d8e6f4', fontSize: 24, lineHeight: 26 },
  languageList: { marginTop: 6 },
  languageListContent: { paddingBottom: 4 },
  languageOption: { minHeight: 44, borderRadius: 10, borderWidth: 1, borderColor: '#1d2b3a', paddingHorizontal: 12, marginBottom: 7, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', backgroundColor: '#0d151f' },
  languageOptionSelected: { borderColor: '#6f8cff', backgroundColor: '#111c31' },
  languageOptionText: { color: '#e6eef7', fontSize: 13, fontWeight: '800' },
  languageOptionCode: { color: '#7890aa', fontSize: 10, fontWeight: '900' },
