...
const GUIDE_KEY = 'scalpAtlas.live.guide.v0414';
...
export default function LiveAnalysisApp() {
  ...
  useEffect(() => {
    AsyncStorage.getItem(GUIDE_KEY)
      .then((value) => {
        if (value !== 'done') setShowGuide(true);
      })
  ...
  const dismissGuide = () => {
    setShowGuide(false);
    void AsyncStorage.setItem(GUIDE_KEY, 'done');
  };
    ...
    <View style={styles.safe}>
      <View style={styles.header}>
        <View>
          ...
          <Text style={styles.subtitle}>LIVE • {SCALP_ATLAS_COUNT} modele • v0.4.14</Text>
        </View>
            ...
            <>
              {showGuide && (
                <View style={styles.guideCard}>
                  <View style={styles.guideHeader}>
                    <View>
                      <Text style={styles.guideTitle}>PORNIRE RAPIDĂ</Text>
                      <Text style={styles.guideSub}>3 pași pentru o analiză LIVE corectă</Text>
                    </View>
                    <Text style={styles.guideVersion}>v0.4.14</Text>
                  </View>
                  <View style={styles.guideStep}><Text style={styles.guideStepNo}>1</Text><Text style={styles.guideStepText}>Îndreaptă camera spre graficul de tranzacționare de pe al doilea dispozitiv.</Text></View>
                  <View style={styles.guideStep}><Text style={styles.guideStepNo}>2</Text><Text style={styles.guideStepText}>Păstrează vizibile lumânările, prețurile și timeframe-ul, apoi fixează cadrul.</Text></View>
                  <View style={styles.guideStep}><Text style={styles.guideStepNo}>3</Text><Text style={styles.guideStepText}>Pornește CAMERA LIVE și așteaptă confirmarea 2/2 înainte de BUY sau SELL.</Text></View>
                  <Pressable style={styles.guideButton} onPress={dismissGuide}>
                    <Text style={styles.guideButtonText}>AM ÎNȚELES</Text>
                  </Pressable>
                </View>
              )}
              ...
              >
                ...
                <View pointerEvents="none" style={[styles.scanFrame, locked && styles.scanFrameLocked]}>
                  <Text style={styles.frameLabel}>{locked ? 'CADRU FIXAT' : 'ÎNCADREAZĂ GRAFICUL AICI'}</Text>
                </View>
              ...
              <View style={[
                ...
                <Text style={styles.chartStatusText}>
                  {analysis?.state === 'INVALID' ? 'GRAFIC NEDETECTAT' : analysis ? 'GRAFIC DETECTAT • ANALIZĂ ACTIVĂ' : 'AȘTEPT GRAFIC PENTRU ANALIZĂ'}
                </Text>
              </View>

              <View style={styles.recentCard}>
                <View style={styles.recentHeader}>
                  <Text style={styles.recentTitle}>ULTIMELE SEMNALE</Text>
              ...
              <Text style={styles.clientFooter}>SCALP ATLAS • v0.4.14 • Semnalele sunt estimări, nu garanții. Decizia de tranzacționare aparține utilizatorului.</Text>
            </>
          )}
        </ScrollView>
      )}
